...
public class XmitTreeItem2 extends TreeItem<String>
{

  private static final FileComparator2 comparator = new FileComparator2 ();
  ...
  // ---------------------------------------------------------------------------------//
  public XmitTreeItem2 (NodeData nodeData)
  // ---------------------------------------------------------------------------------//
  {
    super (nodeData.name);
  ...
  // ---------------------------------------------------------------------------------//
  ObservableList<TreeItem<String>> buildChildren ()
  // ---------------------------------------------------------------------------------//
  {
  ...
  // ---------------------------------------------------------------------------------//
  private void addFiles (ObservableList<TreeItem<String>> children, NodeData nodeData)
  // ---------------------------------------------------------------------------------//
  {
    ...
    for (File file : files)
    {
      ...
      if (file.isDirectory () || NodeData.isValidFileName (file.getName ()))
        children.add (new XmitTreeItem2 (new NodeData (file)));
    }
    Collections.sort (children, comparator);
  }

  // ---------------------------------------------------------------------------------//
  private void addDatasets (ObservableList<TreeItem<String>> children, NodeData nodeData)
  // ---------------------------------------------------------------------------------//
  {
    for (Dataset dataset : nodeData)
      children.add (new XmitTreeItem2 (new NodeData (dataset)));
  }

  // ---------------------------------------------------------------------------------//
  private void addPdsMembers (ObservableList<TreeItem<String>> children,
      NodeData nodeData)
  // ---------------------------------------------------------------------------------//
  {
    for (PdsMember member : nodeData.getPdsXmitMembers ())
      children.add (new XmitTreeItem2 (new NodeData (member)));
  }

  // ---------------------------------------------------------------------------------//
  private void addCompressedFiles (ObservableList<TreeItem<String>> children,
      NodeData nodeData)
  // ---------------------------------------------------------------------------------//
  {
    Map<ZipEntry, XmitTreeItem2> fileList = decompressZip (nodeData.file.toPath ());
    if (fileList.size () > 0)
    {
  ...
  // ---------------------------------------------------------------------------------//
  public static Map<ZipEntry, XmitTreeItem2> decompressZip (Path path)
  // ---------------------------------------------------------------------------------//
  {
    Map<ZipEntry, XmitTreeItem2> fileMap = new HashMap<> ();
    ...
    try (ZipFile zipFile = new ZipFile (path.toString ()))
    {
      ...
      while (entries.hasMoreElements ())
      {
        ...
        else if (NodeData.isValidFileName (entryName))
        {
          ...
          fileMap.put (entry, new XmitTreeItem2 (new NodeData (tmp, entryName)));
        }
        else
